...
async function main() {
  ...
  try {
    ...
    // Evaluate the DOM script EXACTLY as syncShiftsToEstama does
    await page.evaluate(() => {
      const selects = document.querySelectorAll('select');
      selects.forEach(sel => {
        // Try to find the select for 2026-07-22 at 17:00
    ...
    if (saveBtn) {
      ...
      await page.screenshot({ path: 'scratch/estama-after-save.png', fullPage: true });
    } else {
