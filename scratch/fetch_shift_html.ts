...
async function run() {
  ...
  // Test /shift?date=2026-06-22
  console.log('Fetching /shift?date=2026-06-22...')
  const r1 = await session.get('https://my.caskan.jp/shift?date=2026-06-22')
  const html1 = await r1.text()
  console.log('HTML length:', html1.length)
  for (const name of names) {
    const count = (html1.match(new RegExp(name, 'g')) || []).length
    console.log(`- Occurrences of "${name}" in /shift HTML: ${count}`)
  }

  // Test /schedule?date=2026-06-26
  console.log('\nFetching /schedule?date=2026-06-26...')
  const r2 = await session.get('https://my.caskan.jp/schedule?date=2026-06-26')
  const html2 = await r2.text()
  console.log('HTML length:', html2.length)
  for (const name of names) {
    const count = (html2.match(new RegExp(name, 'g')) || []).length
    console.log(`- Occurrences of "${name}" in /schedule HTML: ${count}`)
  }
}
