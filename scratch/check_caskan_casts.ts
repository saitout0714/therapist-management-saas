...
async function getShiftsForDate(session: FetchSession, dateStr: string) {
  const r = await session.get(`https://my.caskan.jp/shift?date=${dateStr}`)
  const html = await r.text()
  const $ = cheerio.load(html)

  const castsWithShifts = new Set<string>()

  $('.tbl-calendar tr').each((i, row) => {
    const nameLink = $(row).find('a[href^="/cast/view"]')
    const castName = nameLink.text().trim()
    if (!castName) return
    castsWithShifts.add(castName)
  })

  return Array.from(castsWithShifts).sort()
}
...
async function run() {
  ...
  const week1Casts = await getShiftsForDate(session, '2026-06-22')
  console.log('\n--- All Cast Names on Caskan (2026-06-22) ---')
  console.log(JSON.stringify(week1Casts, null, 2))

  const week2Casts = await getShiftsForDate(session, '2026-06-29')
  console.log('\n--- All Cast Names on Caskan (2026-06-29) ---')
  console.log(JSON.stringify(week2Casts, null, 2))
}
