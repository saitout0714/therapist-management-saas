...
async function setupShopOwner({ loginId, password, ownerName, role = 'agency_client_owner', shopNames }) {
  ...
  for (const shop of shops) {
    ...
    if (!existing) {
      ...
      console.log(`${shop.name} is already linked to user ${userId}.`);
    }
  }
  ...
  console.log(`Linked shops: ${shops.map(s => s.name).join(', ')}`);
}

const args = process.argv.slice(2);
const loginId = args[0] || 'urban_himitsu';
const password = args[1] || 'urban2026';
const ownerName = args[2] || 'アーバンスパ・新宿秘密妻 オーナー';
const shopNames = args.slice(3).length > 0 ? args.slice(3) : ['アーバンスパ', '新宿秘密妻'];

setupShopOwner({ loginId, password, ownerName, shopNames }).catch(err => {
