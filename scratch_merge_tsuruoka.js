...
async function findAndMergeTsuruoka() {
  console.log("=== Tsuruoka Natsumi 検索・統合開始 ===");
  ...
  const matched = list.filter(t => 
    t.name.toLowerCase().includes('tsuruoka') || 
    t.name.toLowerCase().includes('tusruoka') || 
    t.name.includes('ツルオカ') || 
    t.name.includes('鶴岡')
  );
  ...
  console.log("\n=== Tsuruoka Natsumi 統合完了 ===");
}

findAndMergeTsuruoka();
