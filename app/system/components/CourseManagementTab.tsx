            ...
            {showForm && (
                <form onSubmit={handleSubmit} className="bg-slate-50 p-6 rounded-xl border border-slate-200 mb-6 space-y-4">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                        <div>
                            <label className="block mb-1 text-xs font-semibold text-slate-600">施術時間（分）</label>
                            <input
                                type="number"
                                className="w-full border border-slate-200 rounded-lg px-4 py-2.5 focus:ring-2 focus:ring-indigo-500/50 outline-none"
                                placeholder="例: 60"
                        ...
                        <div>
                            <label className="block mb-1 text-xs font-semibold text-slate-600">基本料金（顧客請求額）</label>
                            ...
                            {courses.map((course) => (
                                <tr key={course.id} className="hover:bg-slate-50/50 transition-colors">
                                    ...
                                    <td className="p-4 text-sm text-right space-x-3 whitespace-nowrap">
                                        ...
                                        >編集</button>
                                        <button className="font-medium text-rose-600 hover:text-rose-800 transition-colors" onClick={() => void handleDelete(course.id)}>削除</button>
                                    </td>
                                </tr>
                            ))}
                            {courses.length === 0 && (
