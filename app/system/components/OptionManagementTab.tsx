...
export function OptionManagementTab() {
    ...
    const [formData, setFormData] = useState<{ name: string, price: number, description: string, is_active: boolean, display_order: number, option_type: 'extension' | 'item', duration_minutes_added: number }>({
        name: '', price: 0, description: '', is_active: true, display_order: 0, option_type: 'extension', duration_minutes_added: 0
    })
    ...
    const resetForm = () => {
        setFormData({ name: '', price: 0, description: '', is_active: true, display_order: 0, option_type: 'extension', duration_minutes_added: 0 })
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-6 md:p-8">
            ...
            {showForm && (
                <form onSubmit={handleSubmit} className="bg-slate-50 p-6 rounded-xl border border-slate-200 mb-6 space-y-4">
                    <input className="w-full border border-slate-200 rounded-lg px-4 py-2.5 focus:ring-2 focus:ring-indigo-500/50 outline-none" placeholder="オプション名" value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} required />
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <select className="w-full border border-slate-200 rounded-lg px-4 py-2.5 focus:ring-2 focus:ring-indigo-500/50 outline-none" value={formData.option_type} onChange={(e) => setFormData({ ...formData, option_type: e.target.value as 'extension' | 'item' })}>
                            <option value="extension">延長</option>
                            <option value="item">アイテム備品等</option>
                        </select>
                        <div className="relative">
                            <span className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 text-sm">¥</span>
                            <input type="number" className="w-full border border-slate-200 rounded-lg pl-8 pr-4 py-2.5 focus:ring-2 focus:ring-indigo-500/50 outline-none" placeholder="追加料金" value={formData.price} onChange={(e) => setFormData({ ...formData, price: Number(e.target.value) })} min={0} />
                        </div>
                    </div>
                    {formData.option_type === 'extension' && (
                        <div className="grid grid-cols-1 gap-4">
                            <input type="number" className="w-full border border-slate-200 rounded-lg px-4 py-2.5 focus:ring-2 focus:ring-indigo-500/50 outline-none" placeholder="追加時間（分）" value={formData.duration_minutes_added} onChange={(e) => setFormData({ ...formData, duration_minutes_added: Number(e.target.value) })} min={0} />
                        </div>
                    )}
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div className="pt-2">
                        <button type="submit" className="px-5 py-2.5 bg-emerald-600 text-white font-medium rounded-xl hover:bg-emerald-700 transition-colors">{editingOption ? '更新する' : '登録する'}</button>
                    </div>
                </form>
            )}
                        ...
                        <tbody className="divide-y divide-slate-100">
                            {options.map((option) => (
                                <tr key={option.id} className="hover:bg-slate-50/50 transition-colors">
                                    ...
                                    <td className="p-4 text-sm text-slate-600">{option.option_type === 'extension' && option.duration_minutes_added > 0 ? `+${option.duration_minutes_added}分` : '-'}</td>
                                    ...
                                    <td className="p-4 text-sm text-right space-x-3 whitespace-nowrap">
                                        <button className="font-medium text-indigo-600 hover:text-indigo-800 transition-colors" onClick={() => { setEditingOption(option); setFormData({ name: option.name, duration_minutes_added: option.duration_minutes_added, price: option.price, description: option.description || '', is_active: option.is_active, display_order: option.display_order, option_type: option.option_type }); setShowForm(true) }}>編集</button>
                            ...
                            {options.length === 0 && <tr><td className="p-8 text-center text-slate-500" colSpan={6}>オプションがありません</td></tr>}
                        </tbody>
                    </table>
                </div>
            </div>
