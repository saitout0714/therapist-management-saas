        ...
        <div className={`p-6 sm:p-8 space-y-6 ${
          ...
          <div className="space-y-4 text-xs">
            <div className={`grid grid-cols-1 sm:grid-cols-3 p-3.5 border ${
              ...
              <span className={`sm:col-span-2 font-semibold ${isCyberTheme ? 'text-white' : 'text-stone-800'}`}>アロマセラピスト・トリートメント施術</span>
            </div>
            <div className={`grid grid-cols-1 sm:grid-cols-3 p-3.5 border ${
              isCyberTheme ? 'bg-[#050014]/80 border-[#ff007f]/30 rounded-xl' : 'bg-[#faf7f0] border-[#d1b464]/20 rounded-sm'
            }`}>
              <span className={`font-bold ${isCyberTheme ? 'text-pink-300' : 'text-stone-500'}`}>資格</span>
              <span className={`sm:col-span-2 font-semibold ${isCyberTheme ? 'text-white' : 'text-stone-800'}`}>18歳以上（高校生不可）、未経験者大歓迎！</span>
            </div>
            <div className={`grid grid-cols-1 sm:grid-cols-3 p-3.5 border ${
              isCyberTheme ? 'bg-[#050014]/80 border-[#ff007f]/30 rounded-xl' : 'bg-[#faf7f0] border-[#d1b464]/20 rounded-sm'
            }`}>
              <span className={`font-bold ${isCyberTheme ? 'text-pink-300' : 'text-stone-500'}`}>給与</span>
              <span className={`sm:col-span-2 font-semibold ${isCyberTheme ? 'text-white' : 'text-stone-800'}`}>日給 30,000円 ～ 80,000円可能（全額日払いOK）</span>
            </div>
            <div className={`grid grid-cols-1 sm:grid-cols-3 p-3.5 border ${
              isCyberTheme ? 'bg-[#050014]/80 border-[#ff007f]/30 rounded-xl' : 'bg-[#faf7f0] border-[#d1b464]/20 rounded-sm'
            }`}>
              <span className={`font-bold ${isCyberTheme ? 'text-pink-300' : 'text-stone-500'}`}>勤務時間</span>
              <span className={`sm:col-span-2 font-semibold ${isCyberTheme ? 'text-white' : 'text-stone-800'}`}>12:00 ～ 翌5:00 (週1日・3時間～OKの自由シフト制)</span>
            </div>
          </div>
