...
const DEFAULT_TEMPLATE = `【ご予約完了】ご予約ありがとうございます。
...
const TAGS = [
  ...
  { tag: '[支払方法]', label: '支払方法' },
  { tag: '[合計料金]', label: '合計料金' },
  { tag: '[決済情報]', label: 'クレジットカード決済情報（クレジット決済時のみ案内表示）' },
  { tag: '[道案内]', label: 'ルームご案内（ルームテンプレート）' },
]

export function WebReserveEmailTemplateTab() {
    ...
    async function loadTemplate() {
      ...
      try {
        const { data, error } = await supabase
          ...
          .select('id, email_template_web_success')
        ...
        if (data) {
          ...
          setTemplate(data.email_template_web_success || DEFAULT_TEMPLATE)
        } else {
          setSettingsId(null)
    ...
    try {
      if (settingsId) {
        const { error } = await supabase
          .from('system_settings')
          .update({
            email_template_web_success: template,
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-4 md:p-5 space-y-6">
      <div>
        <h2 className="text-base font-bold text-slate-800 mb-1">WEB予約完了メールテンプレート設定</h2>
        <p className="text-sm text-slate-500">
          WEB予約の受付時にお客様へ自動送信される、ご予約完了通知メールの本文をカスタマイズできます。
        </p>
      </div>
        ...
        <div className="space-y-4">
          <div>
            ...
            <p className="text-xs text-slate-400 mt-1">
              クリックするとテンプレートのカーソル位置にタグが挿入され、お客様へ送られるメール内で自動置換されます。
            </p>
          </div>
