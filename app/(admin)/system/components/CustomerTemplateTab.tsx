...
const DEFAULT_TEMPLATE = `【ご予約内容のご確認】
...
export function CustomerTemplateTab() {
    ...
    async function loadTemplate() {
      ...
      try {
        ...
        if (data) {
          ...
          setTemplate(data.customer_template || DEFAULT_TEMPLATE)
        } else {
          setSettingsId(null)
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-4 md:p-5 space-y-6">
      <div>
        <h2 className="text-base font-bold text-slate-800 mb-1">お客様連絡テンプレート設定</h2>
        <p className="text-sm text-slate-500">
          予約詳細からお客様への連絡文面（LINE/SMS等へのコピー用）を生成する際のテンプレートをカスタマイズできます。
        </p>
      </div>
