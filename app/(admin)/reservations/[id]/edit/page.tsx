...
export default function EditReservationPage() {
  ...
  const fetchInitialData = async () => {
    ...
    try {
      ...
      // 編集対象の予約の therapist_id も確実に含める
      const { data: currentResData } = await supabase
        .from('reservations')
        .select('therapist_id')
        .eq('id', reservationId)
        .maybeSingle()
      ...
      const targetTherapistIds = Array.from(new Set([
        ...activeTherapistIds,
        ...(currentResData?.therapist_id ? [currentResData.therapist_id] : [])
      ]))
      ...
      if (reservation.notes) {
        ...
        setDispatchType(dispatch.dispatch_type)
        if (dispatch.dispatch_type === 'hotel') {
          setHotelRoomNumber(dispatch.hotel_room_number || '')
          setSelectedRoomId(reservation.room_id || '')
        } else if (dispatch.dispatch_type === 'home') {
          setHomeAddress(dispatch.home_address || '')
          setParkingAvailable(dispatch.parking_available || 'yes')
          setPickupStaff(dispatch.pickup_staff || 'none')
        } else {
          setSelectedRoomId(reservation.room_id || '')
        }
      } else {
