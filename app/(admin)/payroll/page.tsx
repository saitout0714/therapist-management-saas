...
type ReservationWithDetails = {
  ...
  shop_id?: string
  shop?: { id: string; name: string; short_name: string | null } | null
}
...
export default function PayrollPage() {
    ...
    try {
      ...
      const { data: resData, error: resError } = await supabase
        ...
        .select(`
          ...
          shop:shops!reservations_shop_id_fkey(id, name, short_name)
        `)
  ...
  const shopSummaries = useMemo(() => {
    const summaryMap: Record<string, { shopName: string; sales: number; back: number; count: number }> = {}
    calculatedRows.forEach(({ reservation: res, result }) => {
      const shopId = res.shop_id || 'unknown'
      const shopName = res.shop?.name || 'その他'
      if (!summaryMap[shopId]) {
        summaryMap[shopId] = { shopName, sales: 0, back: 0, count: 0 }
      }
      summaryMap[shopId].sales += result.totalPrice
      summaryMap[shopId].back += result.totalBack + (res.is_hime ? (res.hime_bonus || 0) : 0)
      summaryMap[shopId].count += 1
    })
  ...
  const generateLineText = () => {
    ...
    if (calculatedRows.length === 0) {
      ...
      // 店舗ごとに予約をグループ化
      const shopGroups: Record<string, { shopName: string; rows: CalculatedRow[] }> = {}
      calculatedRows.forEach(row => {
        const shopId = row.reservation.shop_id || 'unknown'
        const shopName = row.reservation.shop?.name || 'その他'
        if (!shopGroups[shopId]) {
          shopGroups[shopId] = { shopName, rows: [] }
        }
        shopGroups[shopId].rows.push(row)
      })

      Object.entries(shopGroups).forEach(([shopId, group]) => {
        text += `\n━━━━━━━━━━━━━━━━\n`
        text += `■ 店舗: ${group.shopName}\n`
        text += `━━━━━━━━━━━━━━━━\n`
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="col-span-1 lg:col-span-2 space-y-4 order-last lg:order-first">
              ...
              {shopSummaries.length > 1 && (
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 bg-white border border-slate-200 rounded-2xl p-4 shadow-sm mb-2">
                  {shopSummaries.map((summary) => (
                    <div key={summary.shopName} className="p-3 bg-slate-50 rounded-xl border border-slate-100 flex flex-col justify-between">
                      <div className="text-sm font-bold text-slate-700 mb-2 border-b pb-1 flex justify-between items-center">
                        <span>{summary.shopName}</span>
                        <span className="text-xs font-normal text-slate-500">{summary.count} 件</span>
                      </div>
                        ...
                        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-3 sm:gap-4 mb-3.5 pb-3.5 border-b border-slate-100">
                          <div>
                            <div className="flex items-center gap-1.5 mb-1.5 flex-wrap">
                              ...
                              {res.shop && (
                                <span className="bg-rose-50 text-rose-700 px-1.5 py-0.5 rounded text-[10px] md:text-xs font-bold border border-rose-100">
                                  {res.shop.short_name || res.shop.name}
                                </span>
                              )}
                              {row.fromCache && (
