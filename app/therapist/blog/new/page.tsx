...
export default function NewTherapistBlogPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const { error: insertErr } = await supabase.from('therapist_blogs').insert([
        {
      ...
      setError('保存に失敗しました: ' + (err.message || 'エラー'));
      setSaving(false);
    }
  };
    ...
    <div className="min-h-screen bg-stone-950 text-stone-100 font-serif flex flex-col">
      <header className="bg-stone-900 border-b border-stone-800 p-4 sticky top-0 z-50">
        <div className="max-w-3xl mx-auto flex items-center justify-between">
          ...
          <h1 className="font-bold text-sm text-stone-100">新規写メ日記投稿</h1>
          <div className="w-12" />
        </div>
      </header>
