...
export default function EditReservationPage() {
  ...
  const fetchInitialData = async () => {
    ...
    try {
      const [customersRes, coursesRes, optionsRes, therapistsRes, pricingRes, settingsRes, reservationRes, discountsRes] = await Promise.all([
            ...
            <div className="space-y-6">
              ...
              {!selectedDiscountId && (
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 animate-in fade-in slide-in-from-top-2 duration-400">
                  ...
                  <div>
                    <label className="block text-sm font-semibold text-slate-700 mb-2">負担区分</label>
                    <select
                      value={formData.manual_burden_type}
                      onChange={(e) => setFormData({ ...formData, manual_burden_type: e.target.value as any })}
                      className="w-full px-4 py-3 bg-white border border-slate-200 rounded-xl focus:ring-2 focus:ring-indigo-500/50 outline-none transition-all font-medium"
                    >
                      <option value="shop_only">店舗負担 (100:0)</option>
                      <option value="split">折半 (50:50)</option>
                      <option value="therapist_only">セラピスト負担 (0:100)</option>
                    </select>
                  </div>
                  <div>
