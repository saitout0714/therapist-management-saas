...
export async function syncTherapistToEstama(
  ...
  try {
    ...
    if (estamaTherapistId) {
      editUrl = \\\https://estama.jp/admin/cast_edit/\/\\\;
    } else {
      isNew = true;
      ...
      if (addLink) {
        ...
        if (href) {
          editUrl = href.startsWith('http') ? href : \\\https://estama.jp\\\\\;
        }
      }
    }
    ...
    if (therapist.comment) {
      const commentInput = await page.\\textarea[name="comment"], textarea[name="message"], textarea[name="intro"];
...
export async function syncTherapistToEstheRanking(
  ...
  try {
    ...
    if (rankingTherapistId) {
      editUrl = \\\https://www.esthe-ranking.jp/shop/therapist/edit/\/\\\;
    } else {
      isNew = true;
    }
    ...
    // 一言コメント・自己紹介
    if (therapist.comment) {
      const commentInput = await page.\\textarea[name="comment"], textarea[name="message"], textarea[name="pr"];
      if (commentInput) await commentInput.fill(therapist.comment);
    }
...
fs.writeFileSync('lib/sync/estama-therapist.ts', estama, 'utf8');
fs.writeFileSync('lib/sync/esthe-ranking-therapist.ts', estr, 'utf8');
