...
async function findAndMergeUchiyama() {
  console.log("=== Uchiyama Hinata 検索・統合開始 ===");
  ...
  const matched = list.filter(t => 
    t.name.toLowerCase().includes('uchiyama') || 
    t.name.includes('ウチヤマ') || 
    t.name.includes('内山')
  );
  ...
  console.log("\n=== Uchiyama Hinata 統合完了 ===");
}

findAndMergeUchiyama();
