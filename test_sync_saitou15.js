...
async function run() {
    ...
    // Trigger click via jQuery on document
    await Promise.all([
        page.waitForNavigation({ waitUntil: 'domcontentloaded', timeout: 15000 }).catch(e => console.log('nav error:', e.message)),
        page.evaluate(() => {
            window.\a.btn-default_submit.trigger('click');
        })
    ]);
